...
class Start extends React.Component {
    ...
    onSubmit = ()=>{
        ...
        if (this.validate()){
            ...
            this.displayError(this.state.newGame? 'Veuillez remplir votre nom':'Veuillez indiquer votre nom et l!identification de la salle que vous voulez rejoindre!!')
        }
    }
    ...
    render(){
        if (this.state.serverConfirmed){
            return(
                <Redirect to={`/live/tictac/game?room=${this.state.room}&name=${this.state.name}`} />
            )
        }else{
