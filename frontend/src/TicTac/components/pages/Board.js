...
import qs from 'qs'
const ENDPOINT = 'https://tictactoe.quizapay.com'

class Board extends Component {
  constructor(props){
    super(props)
    this.state = {
      game: new Array(100).fill(null),
      piece: 'X',
      turn: true,
      end: false,
      room: '',
  ...
  componentDidMount() {
    ...
    const {room, name} = qs.parse(window.location.hash, {
  ...
  handleWin(id, gameState) {
    ...
    if (this.socketID === id){
      ...
      this.setState({currentPlayerScore:playerScore, statusMessage:'Vous avez gagnez'})
    }else{
      const opponentScore = this.state.opponentPlayer[1] + 1
      const opponent = this.state.opponentPlayer
      opponent[1] = opponentScore
      this.setState({opponentPlayer:opponent, statusMessage:`${this.state.opponentPlayer[0]} a gagné`})
    }
  ...
  handleDraw(gameState){
    this.setBoard(gameState)
    this.setState({end:true, statusMessage:'Veuillez rejouer!!'})
  }
  ...
  setMessage(){
    const message = this.state.turn?"c'est votre tour":`c'est le tour de ${this.state.opponentPlayer[0]}`
      ...
      return(
        <div className="absolute w-full h-full">
          <div className="container-fluid">
            ...
            <ScoreBoard data={{player1:['Vous!', this.state.currentPlayerScore], player2:[this.state.opponentPlayer[0], this.state.opponentPlayer[1]]}}/>
