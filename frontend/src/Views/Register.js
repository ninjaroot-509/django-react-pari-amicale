...
import { withStyles, TextField, Button, Container, Grid, Typography  } from '@material-ui/core'
...
const Register = ({ history }) => {
    ...
    const handleRegisterSubmit = (ev) => {
        ev.preventDefault()
        if (username === "" || password === "" || password2 === "" || email === "") alert("Remplissez tous les champs!")
        else if (password !== password2) alert("Les mots de passe ne correspondent pas!")
        else {
            setLoad(true)
            const body = JSON.stringify({ username, email, password });
            const config = { headers: { 'Content-Type': 'application/json' } }
            axios.post('https://quizapay.com/api/auth/register', body, config)
                .then(res => {
        ...
        <Grid container justify="center" alignItems="center" spacing={4}>
          ...
          <form onSubmit={handleRegisterSubmit}>
                <center>
                    ...
                    <center>
                        <label htmlFor="Comfirme password">
                        <TextField
                            color="secondary"
                            className={classes.textField}
                            placeholder="Comfirmez votre password"
                            size="small"
                            type="password2"
                            name="password2"
                            variant="outlined"
